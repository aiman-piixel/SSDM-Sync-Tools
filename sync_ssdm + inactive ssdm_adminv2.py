...
def return_day(DaySeries):
    ...
    if DaySeries[x] == "01":
        ...
        select.select_by_value(DaySeries[x])
...
def return_year(YearSeries):
    ...
    if YearSeries[x] == "2023":
        select.select_by_value('2023')
    elif YearSeries[x] == "2024":
...
def return_hour(HourSeries):
    ...
    select.select_by_value(HourSeries[x])
    
    select_AMPM = driver.find_element(By.XPATH, "//select[@name='am_pm']")
    select = Select(select_AMPM)
    if HourSeries[x] == "12":
        select.select_by_value('PM')
    elif HourSeries[x] == "01":
        select.select_by_value('PM')
    elif HourSeries[x] == "02":
        select.select_by_value('PM')
    elif HourSeries[x] == "03":
        select.select_by_value('PM')
    elif HourSeries[x] == "04":
        select.select_by_value('PM')
    elif HourSeries[x] == "05":
        select.select_by_value('PM')
    elif HourSeries[x] == "06":
        select.select_by_value('PM')
    elif HourSeries[x] == "07":
        select.select_by_value('AM')
    elif HourSeries[x] == "08":
        select.select_by_value('AM')
    elif HourSeries[x] == "09":
        select.select_by_value('AM')
    elif HourSeries[x] == "10":
        select.select_by_value('AM')
    elif HourSeries[x] == "11":
        select.select_by_value('AM')
    else:
        select.select_by_value('AM')
...
def return_min(MinSeries):
    ...
    select.select_by_value(MinSeries[x])
    ...
    try:
        ...
        if statusDone[i]==1:
            ...
            for date_string in DateTime:
                ...
                time_match = re.search(r'(\d{2}:\d{2})(:\d{2})? ?(\S+)?', date_parts[1])

                time_str = time_match.group(1)
                seconds = time_match.group(2)[1:] if time_match.group(2) else None
                am_pm_indicator = time_match.group(3)

                # Convert the AM/PM indicator to uppercase if it exists and is not a numeric value
                if am_pm_indicator and am_pm_indicator not in '0123456789':
                    am_pm_indicator = am_pm_indicator.upper() if am_pm_indicator in am_pm_indicators else None
                else:
                ...
                yearArr.append(year)
                hourArr.append(hours)
                minuteArr.append(minutes)
                
                dayValue = pd.array(dayArr)
                monthValue = pd.array(monthArr)
                yearValue = pd.array(yearArr)
                hourValue = pd.array(hourArr)
                minuteValue = pd.array(minuteArr)
            ...
            for x in range(len(student_name)):
                ...
                #select date
                return_day(dayValue)
                return_month(monthValue)
                #select time
                return_hour(hourValue)
                return_min(minuteValue)
                #select teacher
